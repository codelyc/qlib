...
if __name__ == "__main__":
    # 根据传入的参数决定导出哪组特征，默认 ALPHA20
    feature_set = sys.argv[1] if len(sys.argv) > 1 else "ALPHA20"
    if feature_set not in FEATURES:
        feature_set = "ALPHA20"
        
    f_dict = FEATURES[feature_set]
    
    # 提取键和值，转换为双引号的 JSON 字符串格式（供 bash eval export 使用）
    names = list(f_dict.keys())
    exprs = list(f_dict.values())
    
    print(f"export feature_names='{json.dumps(names)}'")
    print(f"export feature_expressions='{json.dumps(exprs)}'")
